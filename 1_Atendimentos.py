"""
pages/1_Atendimentos.py — Gestão de Atendimentos
Exibe dados da API Gove. Permite integrar ao Supabase.
"""
...
from gove import buscar_atendimentos, parse_contribuinte, parse_atendente, parse_setor
...
with fc4:
    ...
    apenas_abertos = st.checkbox("Só abertos", value=st.session_state["apenas_abertos"],
                                  key="w_abertos")
...
def _safe_nunique(df, col):
    ...
    return int(df[col].astype(str).replace("—","").replace("","<null>").nunique())
...
with col_info:
    st.markdown(
        f"**{total:,}** atendimento(s) carregado(s)  ·  "
        f"<span class='live-pill'><span class='dot'></span>AO VIVO</span>  "
        f"<span style='font-size:12px;color:#64748b'>"
        f"{agora.strftime('%d/%m/%Y %H:%M:%S')}</span>",
...
if df.empty:
    ...
    # Busca textual
    busca = st.text_input("busca_tabela", placeholder="🔍 Buscar na tabela...",
