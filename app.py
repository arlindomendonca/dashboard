...
with st.expander("🔧 Diagnóstico da API", expanded=False):
    debug_mode = st.checkbox("Ativar debug (mostra JSON bruto)", value=False, key="debug_mode")
...
def _count_vazio(df, col):
    if df.empty or col not in df.columns: return 0
    return int(df[col].astype(str).isin({"—","-","","None","nan"}).sum())

total_setores = _nunique(df, "Setor")
sem_atendente = _count_vazio(df, "Atendente")
...
if btn_integrar and parsed:
    with st.spinner("Integrando ao Supabase..."):
        res = integrar_tudo(parsed)
    if res.get("ok"):
        st.success(
            f"✅ Integração concluída! "
            f"Contribuintes: **{res['contribuintes']['count']}** · "
...
st.markdown("---")
st.caption(f"🕒 {agora.strftime('%d/%m/%Y %H:%M:%S')} · Cache 5 min · API Gove · Prefeitura de Rio Verde")
