...
if btn_integrar and chats:
    ...
    # Mostra resultado detalhado de cada etapa
    for ent, r in res.items():
        if ent == "ok" or not isinstance(r, dict):
            continue
        if r.get("ok"):
            st.success(f"✅ **{ent}**: {r['count']} registro(s) integrado(s)")
        else:
            st.error(f"❌ **{ent}**: {r.get('error', 'Erro desconhecido')}")
